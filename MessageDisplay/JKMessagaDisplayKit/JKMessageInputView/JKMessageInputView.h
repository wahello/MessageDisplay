...
typedef NS_ENUM(NSInteger, JKMessageInputViewStyle) {
    // 分两种,一种是iOS6样式的，一种是iOS7样式的
    JKMessageInputViewStyleQuasiphysical,
    JKMessageInputViewStyleFlat
};

@protocol JKMessageInputViewDelegate <NSObject>
...
@interface JKMessageInputView : UIImageView

@property (nonatomic, weak) id <JKMessageInputViewDelegate> delegate;

/**
 *  用于输入文本消息的输入框
 */
@property (nonatomic, weak, readonly) JKMessageTextView *inputTextView;


/**
 *  当前输入工具条的样式
 */
@property (nonatomic, assign) JKMessageInputViewStyle messageInputViewStyle;  // default is XHMessageInputViewStyleFlat
