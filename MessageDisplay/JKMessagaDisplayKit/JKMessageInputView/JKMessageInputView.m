...
@interface JKMessageInputView ()<UITextViewDelegate>

@property (nonatomic, weak, readwrite) JKMessageTextView *inputTextView;
 ...
 *  @param style 输入框样式类型
 */
- (void)setupMessageInputViewBarWithStyle:(JKMessageInputViewStyle)style ;
...
@implementation JKMessageInputView
...
- (UIButton *)createButtonWithImage:(UIImage *)image HLImage:(UIImage *)hlImage {
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, [JKMessageInputView textViewLineHeight], [JKMessageInputView textViewLineHeight])];
    if (image)
...
- (void)setupMessageInputViewBarWithStyle:(JKMessageInputViewStyle)style {
    ...
    // 输入框
    CGFloat textViewLeftMargin = ((style == JKMessageInputViewStyleFlat) ? 6.0 : 4.0);
    ...
    CGFloat height = [JKMessageInputView textViewLineHeight];
    
    // 初始化输入框
    JKMessageTextView *textView = [[JKMessageTextView  alloc] initWithFrame:CGRectZero];
    
    // 这个是仿微信的一个细节体验
    textView.returnKeyType = UIReturnKeySend;
    textView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
    
    textView.placeHolder = NSLocalizedStringFromTable(@"发送消息", @"SendAMessage", nil);
    ...
    // 配置不同iOS SDK版本的样式
    switch (style) {
        case JKMessageInputViewStyleQuasiphysical: {
        ...
        case JKMessageInputViewStyleFlat: {
    ...
    if (self.allowsSendVoice) {
        ...
        [button setBackgroundImage:JK_STRETCH_IMAGE([UIImage imageNamed:@"VoiceBtn_Black"], edgeInsets)  forState:UIControlStateNormal];
        [button setBackgroundImage:JK_STRETCH_IMAGE([UIImage imageNamed:@"VoiceBtn_BlackHL"], edgeInsets)  forState:UIControlStateHighlighted];
...
- (void)setup {
    ...
    _messageInputViewStyle = JKMessageInputViewStyleFlat;
}
...
+ (CGFloat)maxheight {
    return ([JKMessageInputView maxLines] + 1.0f) * [JKMessageInputView textViewLineHeight];
}
