//
//  JKMessageTableViewController.m
//  MessageDisplay
//
...
#import "JKVoiceRecordHUD.h"
#import "JKVoiceRecordHelper.h"

@interface JKMessageTableViewController ()
/**
 *  判断是否用户手指滚动
 */
...
@property (nonatomic, assign) JKInputViewType textViewInputViewType;


@property (nonatomic, weak, readwrite) UITableView *messageTableView;
@property (nonatomic, weak, readwrite) JKMessageInputView *messageInputView;
@property (nonatomic, weak, readwrite) JKShareMenuView *shareMenuView;
@property (nonatomic, weak, readwrite) JKEmotionManagerView *emotionManagerView;
@property (nonatomic, strong, readwrite) JKVoiceRecordHUD *voiceRecordHUD;
 ...
 *  管理录音工具对象
 */
@property (nonatomic, strong) JKVoiceRecordHelper *voiceRecordHelper;
 ...
 *  @return 返回计算的高度
 */
- (CGFloat)calculateCellHeightWithMessage:(JKMessage *)message atIndexPath:(NSIndexPath *)indexPath;
 ...
 *  @param photo 目标图片
 */
- (void)didSendMessageWithPhotoArr:(NSArray *)photos;
...
@implementation JKMessageTableViewController
...
- (JKShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        JKShareMenuView *shareMenuView = [[JKShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
        shareMenuView.delegate = self;
        shareMenuView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"inputViewBackPointColor"]];
...
- (JKEmotionManagerView *)emotionManagerView {
    if (!_emotionManagerView) {
        JKEmotionManagerView *emotionManagerView = [[JKEmotionManagerView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), 224)];
        emotionManagerView.delegate = self;
        emotionManagerView.dataSource = self;
        emotionManagerView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"inputViewBackPointColor"]];
...
- (JKVoiceRecordHUD *)voiceRecordHUD {
    if (!_voiceRecordHUD) {
        _voiceRecordHUD = [[JKVoiceRecordHUD alloc] initWithFrame:CGRectMake(0, 0, 140, 140)];
    }
    return _voiceRecordHUD;
}

- (JKVoiceRecordHelper *)voiceRecordHelper {
    if (!_voiceRecordHelper) {
        _isMaxTimeStop = NO;
        
        JKWEAKSELF
        _voiceRecordHelper = [[JKVoiceRecordHelper alloc] init];
...
- (void)finishSendMessageWithBubbleMessageType:(JKBubbleMessageMediaType)mediaType {
    switch (mediaType) {
        case JKBubbleMessageMediaTypeText: {
        ...
        case JKBubbleMessageMediaTypePhoto: {
            break;
        }
        case JKBubbleMessageMediaTypeVideo: {
            break;
        }
        case JKBubbleMessageMediaTypeVoice: {
            break;
        }
        case JKBubbleMessageMediaTypeEmotion: {
            break;
        }
        case JKBubbleMessageMediaTypeLocalPosition: {
            break;
        }
        case JKBubbleMessageMediaTypeCard: {
            break;
        }
        default:
            break;
...
- (void)setup {
    ...
    self.keyboardViewHeight = (JKIsiPad ? 264 : 216);
    _allowsPanToDismissKeyboard = NO;
    _allowsSendVoice = YES;
    _allowsSendMultiMedia = YES;
    _allowsSendFace = YES;
    _inputViewStyle = JKMessageInputViewStyleFlat;
...
//    }
    ...
    CGFloat inputViewHeight = (self.inputViewStyle == JKMessageInputViewStyleFlat) ? 45.0f : 40.0f;
    ...
    if (self.allowsPanToDismissKeyboard) {
        ...
        self.messageTableView.keyboardDidScrollToPoint = ^(CGPoint point) {
            if (weakSelf.textViewInputViewType == JKInputViewTypeText)
    ...
    // 初始化输入工具条
    JKMessageInputView *inputView = [[JKMessageInputView alloc] initWithFrame:inputFrame];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.textViewInputViewType != JKInputViewTypeNormal) {
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
    CGFloat maxHeight = [JKMessageInputView maxheight];
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (hide) {
            switch (self.textViewInputViewType) {
                case JKInputViewTypeEmotion: {
                    EmotionManagerViewAnimation(hide);
                    break;
                }
                case JKInputViewTypeShareMenu: {
                    ShareMenuViewAnimation(hide);
                    break;
                }
                case JKInputViewTypeRecord: {
                    ShareMenuViewAnimation(hide);
                    break;
                }
                default:
...
//                    EmotionManagerViewAnimation(hide);
        ...
        if (hide) {
            self.textViewInputViewType = JKInputViewTypeNormal;
        }
    }];
}

#pragma mark - Voice Recording Helper Method
- (void)prepareRecordWithCompletion:(JKPrepareRecorderCompletion)completion {
...
- (void)inputTextViewWillBeginEditing:(JKMessageTextView *)messageInputTextView {
    self.textViewInputViewType = JKInputViewTypeText;
}

- (void)inputTextViewDidBeginEditing:(JKMessageTextView *)messageInputTextView {
...
- (void)didChangeSendVoiceAction:(BOOL)changed {
    if (changed) {
        if (self.textViewInputViewType == JKInputViewTypeText)
            return;
...
- (void)didSelectedMultipleMediaAction {
    ...
    self.textViewInputViewType = JKInputViewTypeShareMenu;
    [self layoutOtherMenuViewHiden:NO];
}

- (void)didSendFaceAction:(BOOL)sendFace {
    if (sendFace) {
        self.textViewInputViewType = JKInputViewTypeEmotion;
        [self layoutOtherMenuViewHiden:NO];
    } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    JKMessage * message = [self.dataSource messageForRowAtIndexPath:indexPath];
...
//    }
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"%@",message.timestamp];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat calculateCellHeight = 100;
